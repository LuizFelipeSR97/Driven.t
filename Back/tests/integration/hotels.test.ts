...
describe("GET /hotels", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("must respond with status 404 when the user's ticket is local and does not have a hotel", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
    ...
    it("should respond with status 402 when the user's ticket is local with hotel", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
    ...
    it("should respond with status 200 and a list of hotels", async () => {
      ...
      const rooms = [await createRoomWithHotelId(createdHotel.id)];
      let avaliables = 0;
      const accommodation: string[] = [];
      if (rooms) {
        for (let i = 0; i < rooms.length; i++) {
          if (rooms[i].hotelId === createdHotel?.id) {
